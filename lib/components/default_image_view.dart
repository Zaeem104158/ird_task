...
class DefaultImageView extends StatelessWidget {
  ...
  const DefaultImageView({
    ...
    this.placeHolderIcon = minaWillLogoAsset,
    ...
    if(isLocalAsset) {
      ...
      if(isNetworkAssetSvg) {
        ...
        return CachedNetworkImage(
          ...
          fit: boxFit,
          placeholder: (context, url) => _getPlaceHolderImage(),
          errorWidget: (context, url, error) => _getPlaceHolderImage(),
        );
      }
    }
  }
  ...
  Widget _getPlaceHolderImage() {
    if(isLocalAssetSvg || isNetworkAssetSvg) {
      return SvgPicture.asset(placeHolderIcon);
    } else {
      return Image.asset(placeHolderIcon);
    }
  }
}
